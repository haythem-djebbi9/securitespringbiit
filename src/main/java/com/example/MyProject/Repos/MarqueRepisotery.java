...
public interface MarqueRepisotery extends JpaRepository<Marque, Long> {
	List<Marque> findByNomMarque(String nom);
	List<Marque> findByNomMarqueContains(String nom);
	//@Query("select m from Marque m where m.nomMarque like %?1 and m.chiffreaffaire > ?2")
 //   List<Marque> findByNomChiffreAffaire(String nom, Double prix);
